    ...
    public static class ColorHelper
    {
        ...
        public static Color GetForeColorForBackColor(Color backColor)
        {
            ...
            return GetThemeColor(
                closestBack.Distance <= closestFore.Distance * 1.25 // prefer back-fore combination
                    ? BackForeExamples[closestBack.Index].Fore
                    : BackForeExamples[closestFore.Index].Back);

            double DistanceTo(KnownColor c2)
            {
                var hsl2 = GetThemeColor(c2).ToPerceptedHsl();
                return
        ...
        public static Color AdaptColor(Color originalRgb, bool isForeground)
        {
            ...
            double DistanceTo(KnownColor c2)
            {
                var hsl2 = GetInvariantColor(c2).ToPerceptedHsl();
        ...
        public static Color TransformHsl(
            this Color c,
        ...
        {
            var exampleOrig = RgbHsl(GetInvariantColor(exampleName));
            var oppositeOrig = RgbHsl(GetInvariantColor(oppositeName));
            var example = RgbHsl(GetThemeColor(exampleName));
            var opposite = RgbHsl(GetThemeColor(oppositeName));
        ...
        private static Color GetInvariantColor(KnownColor c2) =>
            _invariantTheme?.GetColor(c2) ?? Color.FromKnownColor(c2);

        private static Color GetThemeColor(KnownColor c2) =>
            _theme?.GetColor(c2) ?? Color.FromKnownColor(c2);
    }
}
