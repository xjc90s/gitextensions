...
namespace GitUI.Theming
{
    public class FormThemeEditor : GitExtensionsForm
    {
        ...
        private Control CreateColorPicker<TName>(
            TName colorName,
            Func<Theme, TName, Color> getColor,
            Action<ThemeEditor, TName, Color> setColor,
            Action<ThemeEditor, TName> resetColor)
        {
            ...
            UpdateText();
            _controller.Changed += UpdateText;
            ...
            void UpdateText()
            {
                ...
                if (getColor(_controller, colorName) != getColor(_defaultTheme, colorName))
                {
                    result.AppendLine("*");
        ...
        private void AddButtons()
        {
            CreateButton("Reset all colors", (s, e) =>
            {
                ...
                _resetting = true;
                _editor.ResetAllColors();
                _resetting = false;
            });

            CreateButton("Save", (s, e) =>
            {
                _editor.SaveToFileDialog();
            });

            CreateButton("Load", (s, e) =>
            {
                _editor.LoadFromFileDialog();
            });
        }
